...
export function AppProvider({ children }) {
  ...
  const [catalogReady, setCatalogReady] = useState(musicService.loaded);
  const pendingTrackRef = useRef(null);

  // Load the music catalog once (fetched from /catalog.json). musicService's
  // query methods return empty results until this resolves, so consumers gate
  // their UI on `catalogReady`.
  useEffect(() => {
    let active = true;
    musicService
      .load()
      .then(() => { if (active) setCatalogReady(true); })
      .catch((err) => {
        console.error('Failed to load music catalog', err);
        if (active) setCatalogReady(false);
      });
    return () => { active = false; };
  }, []);
