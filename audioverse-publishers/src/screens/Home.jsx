...
export default function Home() {
  // Recompute once the async catalog finishes loading (it's empty on first
  // render now that the catalog is fetched from /catalog.json rather than imported).
  const { catalogReady } = useApp();
  const radioStations = useMemo(() => musicService.getTopRadioStations(6), [catalogReady]);
  const totalTracks = useMemo(() => musicService.getAllTracks().length, [catalogReady]);
  const totalRadio = useMemo(() => musicService.getRadioStations().length, [catalogReady]);
