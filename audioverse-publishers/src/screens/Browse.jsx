...
export default function Browse() {
  const { catalogReady } = useApp();
  const searchParams = useSearchParams();
  const useCaseParam = searchParams?.get('useCase') ?? null;

  const allPlaylists = useMemo(() => musicService.getAllPlaylists(), [catalogReady]);
  const sponsored = useMemo(() => musicService.getSponsoredPlaylists(), [catalogReady]);
  const trending = useMemo(() => musicService.getTrendingTracks(12), [catalogReady]);
  const mostDownloaded = useMemo(() => musicService.getMostDownloaded(12), [catalogReady]);
  const topRadio = useMemo(() => musicService.getTopRadioStations(6), [catalogReady]);

  const useCaseTracks = useMemo(() => {
    if (!useCaseParam) return null;
    return musicService.getTracksByUseCase(useCaseParam);
  }, [useCaseParam, catalogReady]);
